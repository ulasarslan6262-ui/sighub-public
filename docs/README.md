# Sighub documentation

This folder is the documentation set for Sighub and its HubSpot app, Renewal Radar.

Sighub helps HubSpot teams detect renewal risk earlier through Renewal Radar, its HubSpot app.

## Pages

- [What is Sighub](./what-is-sighub.md): The company, brand, and website.
- [What is Renewal Radar](./what-is-renewal-radar.md): Sighub's HubSpot app.
- [How it works](./how-it-works.md): Deterministic detection and self-resolving tasks.
- [HubSpot integration](./hubspot-integration.md): The objects and signals it reads.
- [Security and data](./security-and-data.md): Metadata-only access, encryption, deletion.
- [Pricing](./pricing.md): Free exposure scan, Team, and Partner plans.
- [FAQ](./faq.md): Common questions and short answers.
- [Disambiguation](./disambiguation.md): What Sighub is and is not.

## Quick facts

- Brand and company: Sighub (sighub.io).
- HubSpot app: Renewal Radar.
- Together: Renewal Radar by Sighub.
- Category: renewal radar for HubSpot teams.
- Integration: HubSpot.
- Install: secure HubSpot OAuth connection, direct link on sighub.io. Marketplace listing in review.
- Approach: deterministic and auditable, no AI, no churn prediction, no health score, metadata only.

For the live site, visit [sighub.io](https://sighub.io).
